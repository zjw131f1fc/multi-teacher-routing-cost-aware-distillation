"""VQA (Visual Question Answering) Dataset 基类。

定义 VQA 类型数据集的通用接口和基础功能。
具体实现应放在 impl/vqa/ 中。
"""
...
class BasePreparer:
    ...
    def __init__(self, config):
        ...
        # 支持分层配置: dataset_settings.vqa_settings 或直接从 dataset_settings 读取（向后兼容）
        if "vqa_settings" in ds_cfg:
            vqa_cfg = ds_cfg["vqa_settings"]
        else:
            vqa_cfg = ds_cfg  # 向后兼容旧格式

        self.split_cfg: Dict[str, Any] = vqa_cfg["split"] if "split" in vqa_cfg else {}
        self.has_category: bool = False
        self.field_map: Dict[str, str] = vqa_cfg["field_map"] if "field_map" in vqa_cfg else {}
    ...
    def split_from_single(self, samples: List[Dict[str, Any]]) -> Tuple[Dict[str, BsesDataset], List[str]]:
        # 获取VQA配置（支持分层和向后兼容）
        ds_cfg = self.config.dataset_settings
        vqa_cfg = ds_cfg.get("vqa_settings", ds_cfg)

        if self.has_category and "category_priority" in vqa_cfg:
            cat_priority = vqa_cfg["category_priority"]
    ...
    def build_split_details(self, split_datasets: Dict[str, BsesDataset], placeholder_splits: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        if "category_priority" in vqa_cfg:
            cat_priority = vqa_cfg["category_priority"]
            priority_defs = cat_priority["values"]
            for item in priority_defs:
                for split_name, mode in item.items():
        ...
        for name in split_datasets.keys():
            ...
            if self.has_category and not is_all and not is_placeholder:
                mode = priority_modes[name] if name in priority_modes else ('origin' if "category_priority" in vqa_cfg else None)
